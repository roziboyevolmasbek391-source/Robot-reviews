...
type TwoGISSessionStatus = {
  ready: boolean;
  storageStatePath: string;
  updatedAt: string | null;
};

export function TwoGISIntegrationPanel() {
  const [sessionStatus, setSessionStatus] = useState<TwoGISSessionStatus | null>(null);
  ...
  async function loadSessionStatus() {
    ...
    setError(null);
    try {
      const response = await fetch('/api/integrations/2gis/session-status');
      const payload = (await response.json()) as TwoGISSessionStatus & { error?: string };

      if (!response.ok) {
        setError(payload.error ?? 'Не удалось проверить браузерную сессию 2GIS');
        return;
      }
  ...
  return (
    <div className="grid gap-4 rounded-xl border border-white/10 bg-white/4 p-4 backdrop-blur-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
         <div>
          <h2 className="text-base font-semibold text-white">2GIS Бизнес</h2>
          <p className="mt-1 text-sm text-slate-400">
            Browser session используется роботом для добавления компаний на карты 2GIS.
          </p>
        </div>
      ...
      <div className="rounded-md border border-white/8 bg-white/5 p-3 text-sm text-slate-300">
        Для автоматического добавления компаний запустите локально <code>npm run 2gis:login</code>, войдите в свой аккаунт 2GIS Partner
